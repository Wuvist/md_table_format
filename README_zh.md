...
## 💡 为什么在浏览器/GitHub 网页预览中可能看起来不齐？

在网页端（如 GitHub Markdown 代码块预览），浏览器默认使用的西文等宽字体（如 `SF Mono`、`Consolas`）没有中文字形，遇到中文时会自动回退到操作系统的无衬线中文字体（如 `PingFang SC` 或 `微软雅黑`）。由于回退字体的物理字宽并不是西文字宽的**严格整数 2.0 倍**（常见为 1.85~2.15 倍），因而在网页纯文本渲染时会出现视觉漂移。

而在 **Sublime Text、VS Code 或终端** 中，只要配置了**真正的中英文等宽字体**，表格就会展现出绝对严格、像素级垂直对其的效果。

### 🎨 编辑器推荐字体配置

推荐在 Sublime Text 中使用专为 CJK 编程设计的等宽字体（如更纱黑体）：

1. **[Sarasa Gothic / 更纱黑体 (Sarasa Mono SC)](https://github.com/be5invis/Sarasa-Gothic)** —— 专为 1:2 中英等宽设计（强烈推荐 ⭐⭐⭐⭐⭐）
2. **[Maple Mono CJK](https://github.com/subframe7536/maple-font)**
3. **[Cascadia Code](https://github.com/microsoft/cascadia-code)**

**Sublime Text 配置方法**：
打开 `Preferences` -> `Settings`，添加：
```json
{
